...
function showHelp(){modal("Cómo jugar",`<div class="help-grid"><article><kbd>A</kbd><kbd>D</kbd><h3>MOVER</h3><p>Camina a izquierda y derecha.</p></article><article><kbd>⇧</kbd><h3>CORRER</h3><p>Más velocidad, consume resistencia.</p></article><article><kbd>ESPACIO</kbd><h3>SALTAR</h3><p>Supera plataformas y peligros.</p></article><article><kbd>S</kbd><h3>AGACHARSE</h3><p>Reduce tu silueta.</p></article><article><kbd>J</kbd><h3>FALDA</h3><p>Ataque amplio y poderoso.</p></article><article><kbd>K</kbd><h3>ABANICO</h3><p>Ataque rápido de alcance.</p></article></div><p class="goal-copy">Llega a las <strong>flores amarillas</strong>, cuida tus tres vidas y deja recuperar la barra de resistencia. P pausa · R reinicia.</p>`);}

function showCredits(){modal("Créditos",`<div class="credits"><img src="./assets/sprites/heroine-atlas.png" alt="Hoja conceptual de animaciones de la protagonista"><div><h3>LA DAMA DE LIMA</h3><p>Diseño, programación, pixel art y música generativa original para esta experiencia.</p><p>Inspirado con respeto en la danza, los balcones, plazas y la memoria visual de Lima.</p><p class="muted">Sin música comercial ni recursos de videojuegos existentes.</p></div></div>`);}

function startGame(id){
  audio.ensure();save.save({currentLevel:id});shell.onkeydown=null;shell.innerHTML=`<section class="play-screen"><div class="game-topbar"><div class="hud-name"><small>JUGADOR</small><b>${safe(save.data.name||"VISITANTE")}</b></div><div><small>VIDAS</small><b id="lives">♥ ♥ ♥</b></div><div><small>PUNTOS</small><b id="score">000000</b></div><div><small>NIVEL</small><b>0${id}</b></div><div class="stamina-wrap"><small>RESISTENCIA</small><div class="meter"><i id="stamina"></i></div></div><div class="attack-ready"><small>ATAQUES</small><b id="cooldowns">J ◆ K ◆</b></div><button id="pause-btn">Ⅱ</button></div><div class="canvas-wrap"><canvas id="game" width="${GAME.width}" height="${GAME.height}" aria-label="Juego de plataformas La Dama de Lima"></canvas><div class="level-banner"><b>0${id} · ${LEVELS[id-1].name}</b><span>${LEVELS[id-1].subtitle}</span></div></div><div class="touch-controls"><div><button data-key="ArrowLeft">◀</button><button data-key="ArrowRight">▶</button></div><div><button data-key="ShiftLeft">RUN</button><button data-key="Space">↑</button><button data-key="KeyJ">J</button><button data-key="KeyK">K</button></div></div><p class="desktop-controls">A D mover · SHIFT correr · ESPACIO saltar · J falda · K abanico · P pausa · R reiniciar</p></section>`;
