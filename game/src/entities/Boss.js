...
export class Boss {
  constructor(data) {
    Object.assign(this, {
      ...
      deathT: 0, attackT: 0, attackDuration: 0, attackType: "guitar", attackCooldown: 1.1, attackIndex: 0, dead: false
    });
  }
  update(dt, player, level) {
    ...
    if (distance < 155 && !this.attackCooldown) {
      ...
      this.attackDuration = this.attackType === "guitar" ? 1.55 : 1.08;
      this.attackT = this.attackDuration; this.attackCooldown = .68; this.animT = 0; this.state = this.attackType;
      return;
    }
    if (distance > 94) {
  ...
  hit(damage, direction) {
    ...
    this.hp -= damage; this.x += direction * 18; this.hurt = .25; this.animT = 0;
  ...
  canDamage() {
    if (this.dying || this.hurt || this.attackT <= 0) return false;
