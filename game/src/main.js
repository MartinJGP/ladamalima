...
const actions={
  new:()=>modal("Nueva partida",`<label class="field">NOMBRE DE LA JUGADORA O JUGADOR<input id="player-name" maxlength="18" autocomplete="off" placeholder="Escribe tu nombre"></label><p class="muted">Tu progreso se guarda en este dispositivo y tus puntuaciones se comparten en el ranking global.</p>`,`COMENZAR`,()=>{const n=document.querySelector("#player-name").value.trim();if(!n)return;save.newGame(n);startGame(1);}),
